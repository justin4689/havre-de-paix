...
<div class="pt-20" style="background-color: var(--color-snow);">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <nav class="flex items-center gap-2 text-sm" style="color: var(--color-slate);">
            <a href="{{ route('home') }}" class="hover:text-orange-500 transition-colors">{{ __('Accueil') }}</a>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div class="lg:col-span-2">
                ...
                <div x-data="{ active: 0, lightbox: false, images: {{ json_encode(array_map(fn($img) => asset($img), $room->images ?: ['images/placeholder.svg'])) }} }"
                    ...
                    <div class="relative aspect-[4/3] sm:aspect-[3/2] rounded-2xl overflow-hidden mb-3 shadow-lg cursor-zoom-in group" @click="lightbox = true">
                        ...
                        <button @click.stop="active = (active - 1 + images.length) % images.length"
                                ...
                                x-show="images.length > 1" aria-label="{{ __('Précédent') }}">
                        ...
                        <button @click.stop="active = (active + 1) % images.length"
                                ...
                                x-show="images.length > 1" aria-label="{{ __('Suivant') }}">
                    ...
                    <div x-show="lightbox" x-transition.opacity x-cloak
                         ...
                         role="dialog" aria-modal="true" aria-label="{{ __('Photos de la chambre') }}">
                        <button @click="lightbox = false" class="absolute top-4 right-4 z-20 text-white/80 hover:text-white cursor-pointer" aria-label="{{ __('Fermer') }}">
                ...
                <div class="mb-6">
                    <div class="flex items-baseline justify-between gap-4 flex-wrap">
                        ...
                        <p class="shrink-0">
                            ...
                            <span class="text-sm ml-1" style="color: var(--color-slate);">{{ __('FCFA / nuit') }}</span>
                        </p>
                    </div>
                ...
                <div class="bg-white rounded-2xl p-6 mb-6 shadow-sm border" style="border-color: var(--color-border);">
                    <h2 class="font-semibold text-lg mb-3" style="color: var(--color-navy); font-family: var(--font-serif);">{{ __('Description') }}</h2>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border mb-6" style="border-color: var(--color-border);">
                    <h2 class="font-semibold text-lg mb-4" style="color: var(--color-navy); font-family: var(--font-serif);">{{ __('Équipements') }}</h2>
                ...
                <div class="rounded-2xl p-5 text-sm flex gap-3" style="background-color: #f0fdf4; border: 1px solid #bbf7d0;">
                    ...
                    <div>
                        <p class="font-medium text-green-800">{{ __("Annulation gratuite jusqu'à 48h avant l'arrivée") }}</p>
                        <p class="text-green-700 mt-1">{{ __("Paiement intégral à l'arrivée — aucun prépaiement en ligne requis.") }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-1">
                <div class="sticky top-24 bg-white rounded-2xl shadow-lg border p-6" style="border-color: var(--color-border);"
                    ...
                    <p class="font-semibold text-base mb-5" style="color: var(--color-navy);">{{ __('Réservez votre séjour') }}</p>

                    {{-- Bloc dates + hôtes soudé --}}
                    <div class="rounded-xl border mb-4 overflow-hidden" style="border-color: #94A3B8;">
                        <div class="grid grid-cols-2">
                            <div class="p-3">
                                <label for="room-check-in" class="block text-[10px] font-bold uppercase tracking-wide" style="color: var(--color-navy);">{{ __('Arrivée') }}</label>
                            ...
                            <div class="p-3 border-l" style="border-color: #94A3B8;">
                                <label for="room-check-out" class="block text-[10px] font-bold uppercase tracking-wide" style="color: var(--color-navy);">{{ __('Départ') }}</label>
                        ...
                        <div class="p-3 border-t" style="border-color: #94A3B8;">
                            <label for="room-guests" class="block text-[10px] font-bold uppercase tracking-wide" style="color: var(--color-navy);">{{ __('Hôtes') }}</label>
                            <select x-model="guests" id="room-guests" class="w-full bg-transparent text-sm outline-none border-0 p-0 cursor-pointer">
                                ...
                                <option value="{{ $i }}">{{ trans_choice(':n hôte|:n hôtes', $i, ['n' => $i]) }}</option>
                                @endfor
                            </select>
                        </div>
                    </div>

                    <a :href="nights > 0 && available ? reservationUrl : '#'"
                       @click.prevent="nights > 0 && available ? window.location.href = reservationUrl : null"
                       class="btn-primary w-full text-base py-3.5"
                       :class="nights === 0 || !available ? 'opacity-50 cursor-not-allowed' : ''">
                        <span x-text="nights === 0 ? '{{ __('Sélectionner les dates') }}' : (available ? '{{ __('Réserver') }}' : '{{ __('Indisponible à ces dates') }}')"></span>
                    </a>

                    <p class="text-center text-xs mt-3" style="color: var(--color-slate);">{{ __("Aucun montant débité aujourd'hui") }} &middot; {{ __('Confirmation instantanée') }}</p>

                    {{-- Détail du prix (calculé côté serveur : saisons + week-ends) --}}
                    <div x-show="nights > 0 && available" class="mt-5 pt-4 border-t" style="border-color: var(--color-border);">
                        <div class="flex justify-between text-sm mb-3" style="color: var(--color-navy);">
                            <span x-text="nights + ' ' + (nights > 1 ? '{{ __('nuits') }}' : '{{ __('nuit') }}')"></span>
                            <span x-show="loading" class="skeleton inline-block w-20 h-4"></span>
                            <span x-show="!loading" x-text="totalPrice.toLocaleString('fr-FR') + ' FCFA'"></span>
                        </div>
                        <div class="flex justify-between font-bold border-t pt-3" style="border-color: var(--color-border); color: var(--color-navy);">
                            <span>{{ __('Total') }} <span class="font-normal text-sm">{{ __("(payé à l'arrivée)") }}</span></span>
                            <span x-show="!loading" x-text="totalPrice.toLocaleString('fr-FR') + ' FCFA'"></span>
                            <span x-show="loading" class="skeleton inline-block w-24 h-5"></span>
                        </div>
                        <p class="text-xs mt-3" style="color: var(--color-slate);">{{ __('Tarifs haute saison (déc–mars, juil–août) et week-end (ven–sam) inclus dans le total.') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        @if ($similar->count() > 0)
        <div class="mt-16">
            <h2 class="section-title mb-8">{{ __('Chambres similaires') }}</h2>
...
function bookingWidget(roomId, pricePerNight) {
    return {
        ...
        // Prix exact côté serveur (saisons + week-ends) via l'API de disponibilité ;
        // estimation locale en attendant la réponse ou en cas d'échec réseau.
        async updatePrice() {
            ...
            this.loading = true;
            try {
                const res = await fetch(`/api/availability?check_in=${this.checkIn}&check_out=${this.checkOut}`, { headers: { Accept: 'application/json' } });
                if (res.ok) {
                    const { data } = await res.json();
                    const room = data.find(r => r.id === this.roomId);
                    if (room) {
                        this.totalPrice = room.price_total;
                    } else {
                        this.available = false;
                    }
                }
            } catch (e) { /* estimation locale conservée */ }
            this.loading = false;
        },
