@extends('layouts.app')
@section('title', __('Réservation confirmée —') . ' ' . $reservation->ref)
...
<div class="pt-24 pb-20 min-h-screen px-4 sm:px-6" style="background-color: var(--color-snow);">
    <div class="max-w-xl mx-auto">
        ...
        <div class="text-center mb-8 animate-fade-up">
            ...
            <p style="color: var(--color-slate);">{{ __('Un email de confirmation a été envoyé à') }} <strong>{{ $reservation->guest_email }}</strong></p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border p-6 mb-4" style="border-color: var(--color-border);">
            <div class="flex items-center justify-between mb-5 pb-4 border-b" style="border-color: var(--color-border);">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-wider mb-1" style="color: var(--color-orange);">{{ __('Référence') }}</p>
            ...
            <div class="space-y-3 text-sm">
                ...
                <div class="flex justify-between">
                    <span style="color: var(--color-slate);">{{ __('Personnes') }}</span>
                ...
                <div class="flex justify-between pt-3 border-t font-semibold text-base" style="border-color: var(--color-border);">
                    <span style="color: var(--color-navy);">{{ __('Total à régler') }}</span>
        ...
        <div class="rounded-xl p-4 mb-4 text-sm flex gap-3" style="background-color: var(--color-sky); color: #0c4a6e;">
            ...
            <div>
                <p class="font-semibold mb-1">{{ __("Paiement à l'arrivée") }}</p>
                <p>{{ __('Le règlement de') }} <strong>{{ number_format($reservation->total_price, 0, ',', ' ') }} FCFA</strong> {{ __("s'effectue directement à l'hôtel à votre arrivée. Aucun montant n'a été prélevé en ligne.") }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border p-5 mb-6" style="border-color: var(--color-border);">
            <h2 class="font-semibold mb-3" style="color: var(--color-navy); font-family: var(--font-serif);">
                {{ __('Comment nous trouver ?') }}
            </h2>
            <p class="text-sm mb-2" style="color: var(--color-slate);">
                <strong>Résidence Hôtel Cascades — Cocody Riviera M'Badon</strong>, {{ __('Abidjan, Côte d\'Ivoire') }}<br>
                {{ __('Depuis le boulevard Mitterrand, direction Riviera 3 puis M\'Badon.') }}
            </p>
            <a href="https://maps.google.com/?q=5.335485805872274,-3.943025171164152" target="_blank" rel="noopener"
        ...
        <p class="text-center text-xs mt-3" style="color: var(--color-slate);">
            {{ __("Le lien d'annulation est également disponible dans votre email de confirmation.") }}
        </p>
    </div>
</div>
@endsection
