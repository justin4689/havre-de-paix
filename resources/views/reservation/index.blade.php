...
@section('title', __('Finaliser ma réservation — Résidence Hôtel Cascades'))
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex items-center justify-center gap-3 mb-10">
            <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-medium hidden sm:block" style="color: #16a34a;">{{ __('Chambre choisie') }}</span>
            </div>
            <div class="h-px w-8 sm:w-12" style="background-color: #16a34a;"></div>
            <div class="flex items-center gap-2">
                <div class="step-dot active">2</div>
                <span class="text-sm font-medium hidden sm:block" style="color: var(--color-orange);">{{ __('Vos coordonnées') }}</span>
            </div>
            <div class="h-px w-8 sm:w-12" style="background-color: var(--color-border);"></div>
            <div class="flex items-center gap-2">
                <div class="step-dot">3</div>
                <span class="text-sm font-medium hidden sm:block" style="color: var(--color-slate);">{{ __('Confirmation') }}</span>
            </div>
        </div>
        ...
        <div class="mb-6 p-4 rounded-xl bg-red-50 border border-red-200">
            <p class="text-sm font-medium text-red-800 mb-2">{{ __('Veuillez corriger les erreurs suivantes :') }}</p>
        ...
        <form action="{{ route('reservation.store') }}" method="POST" id="booking-form" novalidate
            ...
            <div class="bg-white rounded-2xl shadow-sm border p-6 mb-4" style="border-color: var(--color-border);">
                <h2 class="font-bold text-lg mb-5" style="color: var(--color-navy);">1. {{ __('Vos coordonnées') }}</h2>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label for="guest_name" class="form-label">{{ __('Nom complet') }} <span class="text-red-500">*</span></label>
                        <input type="text" name="guest_name" id="guest_name"
                               value="{{ old('guest_name') }}"
                               placeholder="{{ __('Ex : Kofi Atta') }}"
                               class="form-input" autocomplete="name" required>
                        @error('guest_name')<p class="text-xs text-red-600 mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label for="guest_phone" class="form-label">{{ __('Téléphone') }} <span class="text-red-500">*</span></label>
                ...
                <div class="mb-4">
                    <label for="guest_email" class="form-label">{{ __('Email') }} <span class="text-red-500">*</span></label>
                    <input type="email" name="guest_email" id="guest_email"
                           value="{{ old('guest_email') }}"
                           placeholder="votre@email.com"
                           class="form-input" autocomplete="email" required>
                    <p class="text-xs mt-1" style="color: var(--color-slate);">{{ __('La confirmation de réservation sera envoyée à cette adresse.') }}</p>
                    @error('guest_email')<p class="text-xs text-red-600 mt-1">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label for="special_requests" class="form-label">{{ __('Demandes spéciales') }}</label>
                    <textarea name="special_requests" id="special_requests" rows="3"
                              class="form-input resize-none"
                              placeholder="{{ __('Lit bébé, arrivée tardive, vue préférée...') }}">{{ old('special_requests') }}</textarea>
                </div>
            </div>

            {{-- ===== 2. CONFIRMATION ===== --}}
            <div class="bg-white rounded-2xl shadow-sm border p-6 mb-6" style="border-color: var(--color-border);">
                <h2 class="font-bold text-lg mb-5" style="color: var(--color-navy);">2. {{ __('Confirmation') }}</h2>

                {{-- Politique d'annulation --}}
                <div class="rounded-xl p-4 mb-5" style="background-color: var(--color-sky); color: #0c4a6e;">
                    <p class="font-semibold text-sm mb-1">{{ __("Politique d'annulation") }}</p>
                    <p class="text-sm">{{ __("Annulation gratuite jusqu'à") }} <strong>{{ __('48h avant votre arrivée') }}</strong>{{ __('. Au-delà, une nuit de séjour vous sera facturée à l\'arrivée.') }}</p>
                    <p class="text-sm mt-2"><strong>{{ __("Paiement à l'arrivée") }}</strong>{{ __(" — aucun montant n'est prélevé en ligne.") }}</p>
                </div>
                ...
                <label class="flex items-start gap-3 cursor-pointer group">
                    ...
                    <span class="text-sm" style="color: var(--color-slate);">
                        {{ __("J'ai lu et j'accepte les") }} <a href="{{ route('legal') }}#cgv" target="_blank" class="underline transition-colors" style="color: var(--color-blue);">{{ __('conditions générales de vente') }}</a> {{ __('et la') }} <a href="{{ route('legal') }}#confidentialite" target="_blank" class="underline transition-colors" style="color: var(--color-blue);">{{ __('politique de confidentialité') }}</a> {{ __('de la Résidence Hôtel Cascades.') }} <span class="text-red-500">*</span>
                    </span>
                </label>
            </div>
            ...
            <button type="submit" id="submit-btn"
                    ...
                    onclick="this.disabled=true; this.innerHTML='{{ __('Confirmation en cours...') }}'; this.form.submit();">
                ...
                <div class="bg-white rounded-2xl shadow-sm border overflow-hidden" style="border-color: var(--color-border);">
                    ...
                    <div class="p-5">
                        ...
                        <div class="grid grid-cols-2 border-t border-b py-3 mb-4 text-sm" style="border-color: var(--color-border);">
                            <div>
                                <div class="text-[10px] font-bold uppercase tracking-wide" style="color: var(--color-slate);">{{ __('Arrivée') }}</div>
                            ...
                            <div class="border-l pl-4" style="border-color: var(--color-border);">
                                <div class="text-[10px] font-bold uppercase tracking-wide" style="color: var(--color-slate);">{{ __('Départ') }}</div>
                        ...
                        <div class="flex justify-between text-sm mb-2" style="color: var(--color-navy);">
                            <span class="underline">{{ number_format($room->price_per_night, 0, ',', ' ') }} FCFA × {{ $nights }} {{ trans_choice('nuit|nuits', $nights) }}</span>
                        ...
                        <div class="flex justify-between text-sm mb-2" style="color: {{ $adjustment > 0 ? 'var(--color-navy)' : '#16a34a' }};">
                            <span class="underline">{{ $adjustment > 0 ? __('Ajustement haute saison') : __('Réduction basse saison') }}</span>
                        ...
                        <p class="text-xs mt-1.5" style="color: var(--color-slate);">{{ __("Payé en intégralité à l'arrivée") }}</p>
                    </div>
                </div>
