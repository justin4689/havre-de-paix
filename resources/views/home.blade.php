...
@section('description', __('Réservez votre séjour à la Résidence Hôtel Cascades, résidence-hôtel à Cocody, Abidjan, Côte d\'Ivoire. Chambres et suites tout confort dans un cadre calme et verdoyant. Paiement à l\'arrivée.'))
@section('hero_nav', '1')
...
<section class="relative min-h-[85vh] flex items-center justify-center overflow-hidden">
    ...
    <div class="relative z-10 text-center text-white px-4 sm:px-6 max-w-5xl mx-auto pt-24">
        ...
        <p class="text-lg sm:text-xl mb-10 leading-relaxed max-w-2xl mx-auto" style="color: rgba(255,255,255,0.85);">
            {{ __('Au cœur de Cocody, à Abidjan. Chambres et suites dans un cadre calme et verdoyant.') }}
...
<section class="py-20 px-4 sm:px-6 lg:px-8" style="background-color: white;">
    <div class="max-w-7xl mx-auto">
        <div class="text-center mb-14">
            <p class="text-sm font-semibold uppercase tracking-widest mb-3" style="color: var(--color-orange);">{{ __('Notre différence') }}</p>
            <h2 class="section-title">{{ __('Pourquoi choisir la Résidence Hôtel Cascades ?') }}</h2>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            @foreach ([
                ['icon' => 'M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z', 'title' => __('Piscine & jardins'), 'desc' => __('Détendez-vous au bord de la piscine, dans un cadre verdoyant et paisible.')],
                ['icon' => 'M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z', 'title' => __('Emplacement idéal'), 'desc' => __('Au cœur de Cocody, à quelques minutes des commerces, ambassades et du Plateau.')],
                ['icon' => 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z', 'title' => __('Paiement à l\'arrivée'), 'desc' => __('Réservez sans risque. Vous payez uniquement à votre arrivée.')],
                ['icon' => 'M13 10V3L4 14h7v7l9-11h-7z', 'title' => __('Confirmation immédiate'), 'desc' => __('Votre réservation est confirmée en ligne en moins de 5 minutes.')],
            ] as $feat)
    ...
    <div class="max-w-7xl mx-auto">
        <div class="flex items-end justify-between mb-12">
            <div>
                ...
                <p class="section-subtitle max-w-md">{{ __("De la Chambre Standard à la Mini Suite, trouvez l'hébergement qui vous correspond.") }}</p>
            </div>
        ...
        <div x-data="{ tab: '{{ $roomsByCategory->keys()->first() }}' }">
            ...
            <div class="flex flex-wrap gap-2 mb-8" role="tablist" aria-label="{{ __('Catégories de chambres') }}">
                ...
                <div class="bg-white rounded-3xl border shadow-sm overflow-hidden grid grid-cols-1 lg:grid-cols-2" style="border-color: var(--color-border);">
                    ...
                    <div class="p-8 lg:p-12 flex flex-col justify-center">
                        ...
                        <div class="flex flex-wrap gap-3">
                            <a href="{{ route('rooms.show', $room->slug) }}" class="btn-primary">{{ __('Voir & Réserver') }}</a>
                            <a href="{{ route('rooms.index', ['category' => [$categoryKey]]) }}" class="btn-outline">
                                {{ __('Toute la catégorie') }} {{ \App\Models\Room::CATEGORIES[$categoryKey] }}
                            </a>
                        </div>
                    </div>
                </div>
    ...
    <div class="max-w-7xl mx-auto">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div class="text-center lg:text-left">
                ...
                <h2 class="text-4xl font-bold text-white tracking-tight mb-4">{{ __('Visitez avant de réserver') }}</h2>
                <p class="leading-relaxed mb-8 max-w-md mx-auto lg:mx-0" style="color: rgba(255,255,255,0.65);">
                    {{ __("Deux minutes suffisent pour ressentir l'atmosphère des Cascades : les jardins, les chambres, le restaurant — comme si vous y étiez. Activez le son d'un simple clic.") }}
                </p>
            ...
            <div class="flex justify-center gap-5 sm:gap-8">
                @foreach (['video-1.mp4' => __('Visite de la résidence'), 'video-2.mp4' => __('L\'expérience Cascades')] as $file => $label)
...
<section class="py-20 px-4 sm:px-6 lg:px-8" style="background-color: white;">
    <div class="max-w-7xl mx-auto">
        <div class="text-center mb-12">
            ...
            <h2 class="section-title">{{ __('Le décor de votre séjour') }}</h2>
        ...
        $decorSets = [
            [
                ['img' => 'hero-facade-jour-2.jpg',  'label' => __('La façade en journée'),    'alt' => 'L\'architecture de la résidence sous le soleil de Cocody',     'class' => 'col-span-2 md:row-span-2 aspect-[4/3] md:aspect-auto'],
                ['img' => 'facade-piscine-jour.jpg', 'label' => __('L\'entrée'),               'alt' => 'L\'entrée de la résidence et son enseigne en pleine journée',  'class' => 'aspect-square md:aspect-auto'],
                ['img' => 'escalier-jour.jpg',       'label' => __('L\'escalier de verre'),    'alt' => 'L\'escalier de verre baigné de lumière naturelle',             'class' => 'aspect-square md:aspect-auto'],
                ['img' => 'enseigne-crotons.jpg',    'label' => __('L\'enseigne'),             'alt' => 'L\'enseigne de la résidence entourée de crotons',              'class' => 'aspect-square md:aspect-auto'],
                ['img' => 'salon-clair.jpg',         'label' => __('Le salon du lobby'),       'alt' => 'Le salon lumineux du lobby',                                   'class' => 'aspect-square md:aspect-auto'],
                ['img' => 'decor1.jpg',              'label' => __('L\'entrée végétale'),      'alt' => 'L\'entrée et son mur végétal orné de sculptures',              'class' => 'aspect-square md:aspect-auto'],
                ['img' => 'salon-baies.jpg',         'label' => __('Les baies vitrées'),       'alt' => 'Le salon lounge et ses baies vitrées',                         'class' => 'aspect-square md:aspect-auto'],
                ['img' => 'hero-facade-piscine.jpg', 'label' => __('Les balcons sur la piscine'), 'alt' => 'Les balcons de la résidence au-dessus de la piscine',       'class' => 'col-span-2 aspect-[16/9] md:aspect-auto'],
            ],
            [
                ['img' => 'hero-facade-nuit.jpg',   'label' => __('La résidence de nuit'),    'alt' => 'La façade de la Résidence Hôtel Cascades illuminée la nuit',  'class' => 'col-span-2 md:row-span-2 aspect-[4/3] md:aspect-auto'],
                ['img' => 'piscine-nuit.jpg',       'label' => __('La piscine de nuit'),      'alt' => 'La piscine illuminée à la tombée de la nuit',                 'class' => 'aspect-square md:aspect-auto'],
                ['img' => 'piscine-rouge.jpg',      'label' => __('Le bassin au mur rouge'),  'alt' => 'Le bassin et son mur rouge éclairé le soir',                  'class' => 'aspect-square md:aspect-auto'],
                ['img' => 'salon-plantes.jpg',      'label' => __('Le salon en soirée'),      'alt' => 'Le salon, ses plantes et ses lumières en soirée',             'class' => 'aspect-square md:aspect-auto'],
                ['img' => 'entree-sculpture.jpg',   'label' => __('L\'entrée sculptée'),      'alt' => 'L\'entrée et sa sculpture sur pierre le soir',                'class' => 'aspect-square md:aspect-auto'],
                ['img' => 'couloir-vegetal.jpg',    'label' => __('Le couloir végétal'),      'alt' => 'Le couloir et son mur végétal en soirée',                     'class' => 'aspect-square md:aspect-auto'],
                ['img' => 'comptoir-marbre.jpg',    'label' => __('La réception'),            'alt' => 'Le comptoir de la réception en marbre',                       'class' => 'aspect-square md:aspect-auto'],
                ['img' => 'bar-banquette.jpg',      'label' => __('La banquette du bar'),     'alt' => 'La banquette du bar sous les lumières du soir',               'class' => 'col-span-2 aspect-[16/9] md:aspect-auto'],
            ],
        ];
        @endphp
...
<section class="py-20 px-4 sm:px-6 lg:px-8" style="background-color: var(--color-snow);">
    <div class="max-w-7xl mx-auto">
        <div class="text-center mb-12">
            <p class="text-sm font-semibold uppercase tracking-widest mb-2" style="color: var(--color-orange);">{{ __('Saveurs') }}</p>
            <h2 class="section-title">{{ __('Bar & Restaurant') }}</h2>
            <p class="section-subtitle max-w-md mx-auto">{{ __('Du petit-déjeuner au dernier verre — nos espaces gourmands vous accueillent toute la journée.') }}</p>
        </div>
            ...
            <div class="card group">
                <div class="relative aspect-[16/10] overflow-hidden cursor-zoom-in"
                    ...
                    <img src="{{ asset('images/resto2.jpg') }}" alt="La salle du restaurant de la Résidence Hôtel Cascades"
                         class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy">
                    <span class="absolute top-4 left-4 px-3 py-1.5 rounded-full text-xs font-bold text-white" style="background-color: rgba(11,18,21,0.55); backdrop-filter: blur(6px);">{{ __('Le Restaurant') }}</span>
                ...
                <div class="p-6">
                    <h3 class="font-bold text-lg mb-2" style="color: var(--color-navy);">{{ __('Une table généreuse') }}</h3>
                    <p class="text-sm leading-relaxed mb-4" style="color: var(--color-slate);">
                        {{ __('Cuisine ivoirienne et internationale préparée avec des produits frais, servie dans une salle chaleureuse — et de grandes tablées dans le jardin pour vos événements.') }}
                    </p>
                    <div class="flex flex-wrap gap-x-5 gap-y-2 pt-4 border-t text-xs font-medium" style="border-color: var(--color-border); color: var(--color-slate);">
                        <span class="flex items-center gap-1.5">
                            ...
                            {{ __('Petit-déjeuner · Déjeuner · Dîner') }}
                        </span>
                        <span class="flex items-center gap-1.5">
                            ...
                            {{ __('Tablées événements au jardin') }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="card group">
                <div class="relative aspect-[16/10] overflow-hidden cursor-zoom-in"
                    ...
                    <img src="{{ asset('images/bar1.jpg') }}" alt="Le comptoir du bar de la Résidence Hôtel Cascades"
                         class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy">
                    <span class="absolute top-4 left-4 px-3 py-1.5 rounded-full text-xs font-bold text-white" style="background-color: rgba(11,18,21,0.55); backdrop-filter: blur(6px);">{{ __('Le Bar') }}</span>
                    <span class="absolute bottom-4 right-4 w-8 h-8 rounded-full items-center justify-center hidden group-hover:flex" style="background-color: rgba(255,255,255,0.2); backdrop-filter: blur(4px);">
                        <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8V6a2 2 0 012-2h2M4 16v2a2 2 0 002 2h2m8-16h2a2 2 0 012 2v2m-4 12h2a2 2 0 002-2v-2"/></svg>
                    </span>
                </div>
                <div class="p-6">
                    <h3 class="font-bold text-lg mb-2" style="color: var(--color-navy);">{{ __("L'heure du cocktail") }}</h3>
                    <p class="text-sm leading-relaxed mb-4" style="color: var(--color-slate);">
                        {{ __("Cocktails, jus frais et belle sélection de spiritueux au comptoir, dans une ambiance feutrée qui s'anime en soirée — pour nos hôtes comme pour leurs invités.") }}
                    </p>
                    <div class="flex flex-wrap gap-x-5 gap-y-2 pt-4 border-t text-xs font-medium" style="border-color: var(--color-border); color: var(--color-slate);">
                        <span class="flex items-center gap-1.5">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            @foreach ([
                ['img' => 'resto3.jpg', 'label' => __('Le soir venu'),      'alt' => 'Le restaurant en ambiance du soir'],
                ['img' => 'resto1.jpg', 'label' => __('Banquets au jardin'),'alt' => 'Grande tablée dressée dans le jardin pour un événement'],
                ['img' => 'bar3.jpg',   'label' => __('Le bar en soirée'),  'alt' => 'Le bar en soirée'],
                ['img' => 'bar2.jpg',   'label' => __('Côté salon'),        'alt' => 'Le bar vu depuis le salon'],
            ] as $tile)
...
<section class="py-20 overflow-hidden" style="background-color: var(--color-navy);">
    <div class="max-w-5xl mx-auto text-center px-4 sm:px-6 lg:px-8 mb-12">
        ...
        <h2 class="text-4xl font-bold text-white tracking-tight">{{ __('Ce que disent nos hôtes') }}</h2>
    </div>

    @php
    $reviews = [
        ['name' => 'Kofi A.',        'note' => 5, 'context' => __('Séjour en famille · Avril 2026'),      'text' => __('Un séjour parfait en famille. La piscine est superbe et le service impeccable.')],
        ['name' => 'Aminata D.',     'note' => 5, 'context' => __('Voyage d\'affaires · Mars 2026'),      'text' => __('Idéalement situé, calme absolu à deux pas de mes rendez-vous à Cocody. Personnel aux petits soins.')],
        ['name' => 'Sophie M.',      'note' => 5, 'context' => __('Séjour en couple · Février 2026'),     'text' => __('Cadre verdoyant et reposant en pleine ville. Les chambres sont propres et très bien équipées.')],
        ['name' => 'Yao K.',         'note' => 5, 'context' => __('Anniversaire · Mai 2026'),             'text' => __('Réservation en ligne simple, paiement à l\'arrivée rassurant. La Suite Prestige vaut chaque franc.')],
        ['name' => 'Jean-Paul K.',   'note' => 4, 'context' => __('Séminaire d\'entreprise · Janv. 2026'),'text' => __('Idéal pour un séminaire résidentiel. Personnel accueillant et cuisine délicieuse.')],
        ['name' => 'Mariam T.',      'note' => 4, 'context' => __('Week-end entre amies · Juin 2026'),    'text' => __('Cadre superbe et calme rare à Abidjan. Le restaurant mérite le détour, poissons grillés excellents.')],
        ['name' => 'Franck B.',      'note' => 5, 'context' => __('Séjour en famille · Déc. 2025'),       'text' => __('En plein Cocody et pourtant la déconnexion totale. Piscine impeccable, les enfants étaient ravis.')],
        ['name' => 'Awa S.',         'note' => 5, 'context' => __('Voyage solo · Mai 2026'),              'text' => __('L\'annulation gratuite m\'a décidée, l\'accueil m\'a conquise. Chambre parfaite, quartier sûr.')],
    ];
    @endphp
...
<section class="py-20 px-4 sm:px-6 lg:px-8 text-center" style="background-color: white;">
    <div class="max-w-2xl mx-auto">
        <h2 class="section-title mb-4">{{ __('Prêt pour votre escapade ?') }}</h2>
        ...
        <p class="mt-4 text-sm" style="color: var(--color-slate);">{{ __("Annulation gratuite jusqu'à 48h avant l'arrivée") }}</p>
    </div>
</section>
