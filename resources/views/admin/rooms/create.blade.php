            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select name="category" class="form-input">
                        ...
                        <option value="{{ $cat->slug }}" {{ old('category', 'standard') === $cat->slug ? 'selected' : '' }}>{{ $cat->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="form-label">Prix / nuit — basse saison (FCFA) <span class="text-red-500">*</span></label>
                    <input type="number" name="price_per_night" value="{{ old('price_per_night') }}" min="0" step="500" class="form-input" required>
                </div>
                <div>
                    <label class="form-label">Prix / nuit — haute saison (FCFA)</label>
                    <input type="number" name="price_high_season" value="{{ old('price_high_season') }}" min="0" step="500" class="form-input" placeholder="Déc–Mars · Juil–Août">
                </div>
                <div>
                    <label class="form-label">Week-end basse saison (FCFA)</label>
                    <input type="number" name="price_weekend_low" value="{{ old('price_weekend_low') }}" min="0" step="500" class="form-input" placeholder="Ven & sam — défaut : +10 %">
                </div>
                <div>
                    <label class="form-label">Week-end haute saison (FCFA)</label>
                    <input type="number" name="price_weekend_high" value="{{ old('price_weekend_high') }}" min="0" step="500" class="form-input" placeholder="Ven & sam — défaut : +10 %">
                </div>
                <div>
